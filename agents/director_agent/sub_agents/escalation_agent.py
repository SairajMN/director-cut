...
Produce a human-ready ticket and write it to `ticket` state:
  id: "ESC-" + a short timestamp
  title: "<incident> — needs human: <denied action>"
  body: 3 lines — what broke, root cause, the exact action a human must take
  channel: "#broadcast-oncall"
  escalated: true

Never propose executing the denied action yourself. Output the ticket JSON only.
"""
